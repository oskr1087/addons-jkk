...
class PlanningApplyService:
    ...
    def run(self):
        snapshot = self.env["mrp.planning.snapshot"].create(
            {
                ...
                "payload": {
                    ...
                    "supply_ids": self.plan.supply_ids.ids,
                },
            }
        )
        ...
        for supply in self.plan.supply_ids.filtered(
            lambda row: row.state != "applied" and row.supply_type in ("make", "buy")
        ):
            if supply.supply_type == "make" and getattr(
                self.plan, "include_manufacturing", True
            ):
                ...
                if supply.production_proposal_id:
                    supply.production_proposal_id.write(
                        {"production_id": production.id, "state": "applied"}
                    )
                productions |= production
            elif supply.supply_type == "buy" and getattr(
                self.plan, "include_purchase", True
            ):
        ...
        snapshot.write(
            {
                ...
                "purchase_ids": [(6, 0, purchases.ids)],
            }
        )
    ...
    def _get_or_create_mo(self, supply):
        ...
        bom = (
            proposal.bom_id
            if proposal and proposal.bom_id
            else find_bom(
                self.env, supply.product_id, company_id=self.plan.company_id.id
            )
        )
        ...
        vals = {
            ...
            "planning_production_proposal_id": proposal.id if proposal else False,
        }
    ...
    def _get_or_create_po(self, supply):
        ...
        seller = (
            proposal.supplierinfo_id
            if proposal
            else supply.product_id.seller_ids.filtered(
                lambda s: not s.company_id or s.company_id == self.plan.company_id
            )[:1]
        )
        if not seller:
        ...
        if line:
            ...
            price_unit = seller.product_uom_id._compute_price(
                seller.price_discounted, supply.product_id.uom_id
            )
