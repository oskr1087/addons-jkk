...
class DemandEngine:
    ...
    def run(self):
        ...
        Line.search([('plan_id', '=', self.plan.id), ('source_type', '=', 'sale')]).unlink()
        domain = [
        ...
        for sale_line in sale_lines:
            if sale_line.order_id.warehouse_id and sale_line.order_id.warehouse_id != self.plan.warehouse_id:
                continue
            ...
            Demand.create({
                ...
                'source_reference': '%s / %s' % (sale_line.order_id.name, sale_line.name),
            })
        for (product_id, date_key), values in grouped.items():
            product = self.plan.env['product.product'].browse(product_id)
            line = Line.search([('plan_id', '=', self.plan.id), ('product_id', '=', product_id), ('date_required', '=', date_key)], limit=1)
            vals = {
                'plan_id': self.plan.id,
                ...
                'source_type': 'sale',
                'source_reference': ', '.join(sorted(set(s.order_id.name for s in values['sources']))),
                'state': 'draft',
            }
