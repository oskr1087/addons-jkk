...
class ConflictResolutionEngine:
    ...
    def run(self):
        self.plan.conflict_ids.filtered(
            ...
            in ("capacity_shortage", "calendar_conflict", "date_conflict")
        ).unlink()
    ...
    def _check_overlaps(self):
        ...
        for operation in self.plan.operation_ids.filtered(
            lambda item: item.state != "cancelled" and item.date_start and item.date_end
        ):
            by_workcenter[operation.workcenter_id.id].append(operation)
        for operations in by_workcenter.values():
            ordered = sorted(
                operations, key=lambda item: (item.date_start, item.date_end)
            )
    ...
    def _check_dependencies(self):
        for operation in self.plan.operation_ids.filtered(
            lambda item: item.date_start and item.date_end
        ):
            parent = operation.production_proposal_id
            if not parent:
                continue
            children = self.plan.operation_ids.filtered(
                lambda item: item.production_proposal_id == parent
                and item.sequence < operation.sequence
            )
    ...
    def _check_late_operations(self):
        for operation in self.plan.operation_ids.filtered(
            lambda item: item.date_end and item.production_proposal_id
        ):
            if (
                operation.production_proposal_id.date_required
                and operation.date_end > operation.production_proposal_id.date_required
            ):
                self.env["mrp.planning.conflict"].create(
                    {
                        "plan_id": self.plan.id,
