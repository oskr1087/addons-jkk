...
class SimplePlanningEngine:
    ...
    def _selected_sale_outgoing_by_warehouse(self, grouped, warehouses):
        """Open outgoing stock moves generated by the exact SO lines in this plan.

        ``virtual_available`` already subtracts outgoing stock moves, but not
        every confirmed sale line is necessarily represented there (routes,
        custom procurement holds, missing rules, etc.). APS therefore plans
        explicit sale demand and adds back only the part that Odoo has already
        subtracted before applying that demand once.

        Formula used later:
            virtual_available
            + selected demand already present in outgoing
            - explicit selected demand

        This is safe in both cases:
        - SO demand already in Odoo forecast -> it is not double-subtracted.
        - SO demand not in Odoo forecast -> APS still detects the shortage.
        """
        ...
        for move in moves:
            ...
            if qty > 1e-6:
                result[(move.product_id.id, warehouse.id)] += qty
    ...
    def run(self):
        ...
        # keep every line that already generated a real document. Those
        # documents remain authoritative supply and are read again through
        # Odoo forecast/open MO/RFQ logic. Only still-unexecuted proposal lines
        # are rebuilt, so recalculation never recreates or duplicates previous
        # manufacturing, purchases or generated transfers.
        executed_lines = self.plan.line_ids.filtered(
            lambda line:
                bool(line.created_production_id)
                or bool(line.created_purchase_line_id)
                or bool(line.created_picking_ids)
        )
        pending_lines = self.plan.line_ids - executed_lines

        # Remove only pending suggestions/proposals.
        self.plan.external_move_ids.filtered(
        ...
        selected_sale_outgoing = self._selected_sale_outgoing_by_warehouse(
            grouped, warehouses
        )
        ...
        for product in products.sorted(key=lambda p: (p.default_code or '', p.display_name, p.id)):
            ...
            for warehouse in warehouses:
                ...
                if self.plan.plan_type == 'purchase':
                    ...
                    # Manufacturing demand comes from the exact SO lines
                    # selected by APS. Odoo's virtual forecast may or may not
                    # already contain their outgoing moves. Add back only the
                    # represented part and subtract the explicit APS demand
                    # exactly once.
                    local_demand = (
                        source['by_warehouse'].get(warehouse.id, 0.0)
                        + component_demand
                ...
                details.append({
                    ...
                    'selected_sale_outgoing': selected_sale_outgoing[key],
                    'rfq': draft_purchase,
