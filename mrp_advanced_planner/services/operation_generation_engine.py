...
class OperationGenerationEngine:
    ...
    def run(self):
        ...
        for proposal in self.plan.production_proposal_ids.filtered(lambda item: item.state != 'cancelled'):
            if not proposal.bom_id:
                continue
            for routing in proposal.bom_id.operation_ids.sorted('sequence'):
                workcenter = routing.workcenter_id
                if not workcenter:
                    self.env['mrp.planning.conflict'].create({'plan_id': self.plan.id, 'conflict_type': 'calendar_conflict', 'severity': 'error', 'product_id': proposal.product_id.id, 'message': 'Routing operation has no work center.'})
                    continue
