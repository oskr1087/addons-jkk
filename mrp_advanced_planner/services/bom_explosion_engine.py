...
class BomExplosionEngine:
    ...
    def _find_bom(self, product):
        ...
        if key not in self._boms:
            self._boms[key] = find_bom(self.env, product, company_id=self.plan.company_id.id)
    ...
    def _explode_node(self, product, quantity, date_required, level=0, parent=None):
        ...
        for bom_line in bom.bom_line_ids:
            ...
            if len(self._seen) >= self.plan.max_requirements:
                raise UserError('The requirement safety limit was reached for this planning run.')
            ...
            if child_bom:
                self._explode_node(component, component_qty, requirement.date_required, level + 1, requirement)

    def run(self):
        self.plan.requirement_ids.unlink()
        for line in self.plan.line_ids.filtered(lambda item: item.product_id and item.net_requirement_qty > 0):
            self._explode_node(line.product_id, line.net_requirement_qty, line.date_required)
