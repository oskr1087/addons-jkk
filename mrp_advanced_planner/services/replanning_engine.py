...
class ReplanningEngine:
    ...
    def run(self):
        operations = self.plan.operation_ids.filtered(lambda row: row.state != 'cancelled')
        ...
        for workcenter, rows in by_workcenter.items():
            ordered = sorted(rows, key=lambda row: (-int(row.production_proposal_id.priority or 0), row.date_end or '', row.id))
            cursor = None
            for sequence, operation in enumerate(ordered, 1):
                if cursor and operation.date_start < cursor:
                    duration = operation.load_hours
                    operation.write({'date_start': cursor, 'date_end': cursor + self._hours(duration)})
