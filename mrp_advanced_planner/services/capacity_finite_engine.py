...
class CapacityFiniteEngine:
    ...
    def run(self):
        operations = self.plan.operation_ids.filtered(lambda operation: operation.state != 'cancelled')
        self.plan.load_ids.unlink()
        self.plan.conflict_ids.filtered(lambda conflict: conflict.conflict_type in ('capacity_shortage', 'calendar_conflict')).unlink()
        ...
        for workcenter_id, workcenter_operations in grouped.items():
            ...
            for operation in workcenter_operations:
                available = self._available_hours(workcenter, operation.date_start, operation.date_end)
                ...
                loads.append(load)
                if not operation.date_start or not operation.date_end or available < operation.load_hours:
                    operation.action_mark_conflict()
                    conflict_type = 'calendar_conflict' if not operation.date_start or not operation.date_end or available <= 0 else 'capacity_shortage'
    ...
    def _available_hours(self, workcenter, start, end):
        ...
        try:
            data = calendar.get_work_duration_data(start_dt, end_dt, compute_leaves=True)
