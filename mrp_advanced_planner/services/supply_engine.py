...
class SupplyEngine:
    ...
    def _available(self, product):
        return self.cache.stock_available(
            product, self.plan.company_id.id, self.plan.warehouse_id.id
        )

    def _vendor(self, product, date_required):
        required_date = (
            fields.Date.to_date(date_required)
            if date_required
            else fields.Date.context_today(product)
        )
        sellers = product.seller_ids.filtered(
            lambda seller: (
                not seller.company_id or seller.company_id == self.plan.company_id
            )
    ...
    def run(self):
        ...
        for requirement in self.plan.requirement_ids.sorted(
            key=lambda item: (item.date_required, item.level, item.id)
        ):
            kind, available = self._route(requirement)
            net = max(requirement.required_qty - available, 0.0)
            requirement.write(
                {"available_qty": available, "net_qty": net, "supply_type": kind}
            )
            if kind == "available":
                continue
            if kind == "blocked":
    ...
    def _build_proposals(self):
        ...
        for supply in self.plan.supply_ids.filtered(
            lambda item: item.supply_type in ("make", "buy")
        ):
            if supply.supply_type == "make":
                production_groups[(supply.product_id.id, supply.date_required)].append(
                    supply
                )
            else:
                ...
                if vendor:
                    purchase_groups[
                        (
                            supply.product_id.id,
                            vendor.partner_id.id,
                            supply.date_required,
                        )
                    ].append((supply, vendor))
        for (product_id, date_required), supplies in production_groups.items():
            product = self.env["product.product"].browse(product_id)
            bom = self._find_bom(product)
            proposal = production_model.create(
                {
                    "name": "MO Proposal - %s" % product.display_name,
                    "plan_id": self.plan.id,
                    "product_id": product.id,
                    "bom_id": bom.id if bom else False,
                    "quantity": sum(item.quantity for item in supplies),
        ...
        for (product_id, vendor_id, date_required), entries in purchase_groups.items():
            ...
            proposal = purchase_model.create(
                {
                    "name": "PO Proposal - %s" % product.display_name,
                    "plan_id": self.plan.id,
                    "product_id": product.id,
                    "vendor_id": vendor.id,
                    "supplierinfo_id": supplierinfo.id if supplierinfo else False,
                    "quantity": sum(item.quantity for item, _vendor in entries),
                    "date_required": date_required,
                    "date_planned": date_required - timedelta(days=delay),
