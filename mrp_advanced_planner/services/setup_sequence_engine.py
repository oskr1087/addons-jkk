...
class SetupSequenceEngine:
    ...
    def run(self):
        ...
        for operation in self.plan.operation_ids.filtered(lambda item: item.state != 'cancelled'):
            by_workcenter[operation.workcenter_id.id].append(operation)
        for operations in by_workcenter.values():
            ordered = sorted(operations, key=lambda item: (item.date_start or '', -getattr(item.production_proposal_id, 'priority', 0), item.sequence, item.id))
            ...
            for sequence, operation in enumerate(ordered, 1):
                ...
                if previous_product and previous_product != operation.product_id:
                    ...
                    _capacity, setup_minutes, _cleanup_minutes = operation.workcenter_id._get_capacity(
