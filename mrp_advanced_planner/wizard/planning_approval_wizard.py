...
class MrpPlanningApprovalWizard(models.TransientModel):
    ...
    product_count = fields.Integer(
        compute="_compute_summary", string="Líneas a fabricar"
    )
    total_qty = fields.Float(compute="_compute_summary", string="Cantidad total")
    production_count = fields.Integer(
        compute="_compute_summary", string="Órdenes de fabricación"
    )

    @api.depends("plan_id", "plan_id.line_ids.planned_production_qty")
    def _compute_summary(self):
        for wizard in self:
            lines = wizard.plan_id.line_ids.filtered(
                lambda line: line.planned_production_qty > 0
            )
