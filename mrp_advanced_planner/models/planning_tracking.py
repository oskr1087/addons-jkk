...
class PlanningSupply(models.Model):
    ...
    production_proposal_id = fields.Many2one('mrp.planning.production.proposal', ondelete='set null', index=True)
    purchase_proposal_id = fields.Many2one('mrp.planning.purchase.proposal', ondelete='set null', index=True)
    state = fields.Selection([('draft', 'Draft'), ('applied', 'Applied'), ('cancelled', 'Cancelled')], default='draft')
...
class PlanningConflict(models.Model):
    ...
    plan_id = fields.Many2one('mrp.planning.plan', required=True, ondelete='cascade', index=True)
    conflict_type = fields.Selection([('material_shortage', 'Material Shortage'), ('capacity_shortage', 'Capacity Shortage'), ('date_conflict', 'Date Conflict'), ('bom_missing', 'BOM Missing'), ('calendar_conflict', 'Calendar Conflict')], required=True)
    severity = fields.Selection([('info', 'Info'), ('warning', 'Warning'), ('error', 'Error')], default='warning', required=True)
    product_id = fields.Many2one('product.product')
    workcenter_id = fields.Many2one('mrp.workcenter')
    operation_id = fields.Many2one('mrp.planning.operation', ondelete='cascade', index=True)
...
class PlanningRun(models.Model):
    ...
    plan_id = fields.Many2one('mrp.planning.plan', required=True, ondelete='cascade', index=True)
    run_type = fields.Selection([('calculation', 'Calculation'), ('simulation', 'Simulation'), ('application', 'Application')], required=True)
    state = fields.Selection([('queued', 'Queued'), ('running', 'Running'), ('completed', 'Completed'), ('failed', 'Failed'), ('cancelled', 'Cancelled')], default='queued', required=True)
