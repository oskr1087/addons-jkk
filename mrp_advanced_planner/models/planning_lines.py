...
class PlanningPlanLine(models.Model):
    ...
    sale_line_id = fields.Many2one(
        "sale.order.line",
        string="Línea de venta principal",
        index=True,
        ondelete="set null",
    )
    ...
    sale_order_count = fields.Integer(
        compute="_compute_source_counts", string="Pedidos"
    )
    sale_line_count = fields.Integer(
        compute="_compute_source_counts", string="Líneas de venta"
    )
    ...
    warehouse_detail_ids = fields.One2many(
        "mrp.planning.plan.line.warehouse",
        "planning_line_id",
    ...
    source_type = fields.Selection(
        [("sale", "Ventas"), ("manual", "Manual"), ("mrp", "Fabricación")],
        default="sale",
    )
    ...
    created_production_id = fields.Many2one(
        "mrp.production", string="Orden de fabricación", copy=False, readonly=True
    )
    ...
    created_picking_ids = fields.Many2many(
        "stock.picking",
        "mrp_planning_line_picking_rel",
        "planning_line_id",
        "picking_id",
        string="Reabastecimientos",
        copy=False,
        readonly=True,
    )
        ...
        for line in self:
            ...
            for sale_line in source_lines:
                ...
                pending_sale_uom = max(
                    ...
                    - (sale_line.qty_delivered or 0.0),
                    0.0,
                )
                pending_product_uom = (
                ...
                rows.append(
                    {
                        ...
                        "delivery_date": (
                            fields.Datetime.to_string(row["delivery_date"])
            ...
            line.sale_order_popover_data = json.dumps(
                {
                    "count": len(rows),
                    "total_pending_qty": round(
                        sum(row["pending_qty"] for row in rows), 2
                    ),
    ...
    def _onchange_action_purchase(self):
        if self.action_purchase:
            ...
            if not self.purchase_vendor_id and self.product_id:
                sellers = (
                    self.product_id.with_company(self.plan_id.company_id)
                    .seller_ids.filtered(
                        lambda seller: not seller.company_id
                        or seller.company_id == self.plan_id.company_id
                    )
        ...
        for line in self:
            if (
                sum(
                    bool(value)
                    for value in (
                        line.action_manufacture,
                        line.action_purchase,
                        line.action_move,
                    )
                )
                > 1
                ...
                raise ValidationError(
                    _(
                        "Solo puede seleccionar una acción por producto: Fabricar, Comprar o Mover."
                    )
                )
    ...
    def _check_planner_qty(self):
        for line in self:
            if line.planner_production_qty < 0:
                raise ValidationError(
                    _("La cantidad planificada no puede ser negativa.")
                )

    def unlink(self):
        protected = self.filtered(
            lambda line: line.created_production_id
        ...
        if protected:
            raise UserError(
                _(
                    "No puede eliminar una línea que ya generó fabricación, compra o reabastecimiento."
                )
            )
        return super().unlink()
    ...
    def action_open_warehouse_stock_detail(self):
        self.ensure_one()
        view = self.env.ref(
            "mrp_advanced_planner.view_mrp_planning_line_warehouse_list"
        )
        return {
...
class PlanningPlanLineWarehouse(models.Model):
    ...
    plan_id = fields.Many2one(
        related="planning_line_id.plan_id", store=True, index=True
    )
    product_id = fields.Many2one(
        related="planning_line_id.product_id", store=True, index=True
    )
    warehouse_id = fields.Many2one(
        "stock.warehouse", required=True, index=True, ondelete="cascade"
    )
    demand_qty = fields.Float(string="Demanda")
