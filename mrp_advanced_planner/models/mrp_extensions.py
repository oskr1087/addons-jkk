...
class MrpProduction(models.Model):
    ...
    advanced_plan_id = fields.Many2one(
        "mrp.planning.plan",
        string="Planificación de origen",
        index=True,
        copy=False,
        readonly=True,
    )
    planning_plan_line_id = fields.Many2one(
    ...
    planning_sale_line_id = fields.Many2one(
        "sale.order.line",
    ...
    planning_production_proposal_id = fields.Many2one(
        "mrp.planning.production.proposal",
        string="Propuesta de planificación",
        index=True,
        copy=False,
        readonly=True,
    )
    ...
    def action_open_advanced_plan(self):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _("Planificación de origen"),
            "res_model": "mrp.planning.plan",
            "view_mode": "form",
            "views": [(False, "form")],
            "res_id": self.advanced_plan_id.id,
            "target": "current",
        }
...
class StockMove(models.Model):
    ...
    def action_create_component_purchase(self):
        ...
        if not production:
            raise UserError(
                _(
                    "Esta línea no pertenece a los componentes de una orden de fabricación."
                )
            )
        if production.state in ("done", "cancel"):
            raise UserError(
                _(
                    "No puede generar una compra desde una orden de fabricación terminada o cancelada."
                )
            )
        ...
        if not self.planning_purchase_vendor_id:
            raise UserError(
                _("Seleccione un proveedor para el componente %s.")
        ...
        if qty <= 0:
            raise UserError(
                _("El pronóstico ya cubre el componente %s; no es necesario comprar.")
                % self.product_id.display_name
            )
        ...
        if not po:
            vals = {
                ...
                "mrp_production_id": production.id,
                "advanced_plan_id": (
                    production.advanced_plan_id.id
                    if production.advanced_plan_id
                    else False
                ),
            }
            if picking_type:
