...
class PhysicalRollAssignment(models.Model):
    ...
    roll_id = fields.Many2one(
        "mrp.physical.roll", required=True, ondelete="cascade", index=True
    )
    ...
    def _compute_remaining(self):
        for assignment in self:
            assignment.remaining_qty = max(
                assignment.quantity - assignment.consumed_qty - assignment.waste_qty,
                0.0,
            )
    ...
    def action_release(self):
        for assignment in self:
            ...
            assignment.write(
                {"state": "released", "date_released": fields.Datetime.now()}
            )
        return True
    ...
    def action_consume(self):
        for assignment in self:
            ...
            assignment.write(
                {
                    "state": "consumed",
                    "consumed_qty": assignment.quantity - assignment.waste_qty,
                }
            )
    ...
    def action_record_waste(self, quantity):
        for assignment in self:
            if quantity < 0 or quantity > assignment.quantity - assignment.consumed_qty:
                raise UserError(
                    "Waste quantity exceeds the remaining assigned quantity."
                )
...
class PhysicalRollCut(models.Model):
    ...
    assignment_id = fields.Many2one(
        "mrp.physical.roll.assignment", ondelete="set null", index=True
    )
