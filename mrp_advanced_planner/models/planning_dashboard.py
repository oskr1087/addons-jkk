...
class PlanningDashboard(models.AbstractModel):
    ...
    def get_data(
        ...
        can_manufacturing = user.has_group(
            "mrp_advanced_planner.group_planner_manufacturing_user"
        )
        can_purchase = user.has_group(
        ...
        manufacturing_plans = scoped_plans.filtered(
            lambda p: p.plan_type == "manufacturing"
        )
        ...
        lines = (
            scoped_plans.mapped("line_ids")
            if scoped_plans
            else self.env["mrp.planning.plan.line"]
        )
        ...
        for line in active_lines.sorted(
            key=lambda l: (
                l.date_required or l.plan_id.date_end,
            ...
            forecast_products.append(
                {
                    ...
                    "delivery_date": (
                        fields.Datetime.to_string(line.date_required)
                    ...
                    "action": (
                        ...
                        if line.action_manufacture
                            ...
                            "purchase"
                            if line.action_purchase
                            else "move" if line.action_move else "none"
                        )
                    ),
                    "action_label": line.action_label,
                    "warehouse": (
                        line.target_warehouse_id.display_name
        ...
        manufacturing_orders = [
            ...
            for mo in productions.sorted(key=lambda m: (m.state, m.id), reverse=True)[
                :30
            ]
        ]
        ...
        if manufacturing_plans:
            ...
            workcenters = workorders.mapped("workcenter_id").sorted(
                key=lambda wc: (wc.sequence, wc.id)
            )
            workcenter_data = self._serialize_workcenters_multi(
                workcenters, manufacturing_plans
            )
        ...
        purchase_orders = [
            {
                ...
                "state": po.state,
                "state_label": dict(Purchase._fields["state"].selection).get(
                    po.state, po.state
                ),
        ...
        transfers = [
            {
                ...
                "plan": (
                    picking.advanced_plan_id.name if picking.advanced_plan_id else ""
                ),
            }
        ...
        total_sales_qty = (
            sum(scoped_plans.mapped("total_sales_qty")) if scoped_plans else 0.0
        )
        total_stock_qty = (
            sum(scoped_plans.mapped("total_stock_qty")) if scoped_plans else 0.0
        )
        total_open_mo_qty = (
            sum(manufacturing_plans.mapped("total_open_mo_qty"))
            if manufacturing_plans
            else 0.0
        )
        total_to_manufacture_qty = (
            sum(manufacturing_plans.mapped("total_to_manufacture_qty"))
            if manufacturing_plans
            else 0.0
        )
        total_to_purchase_qty = (
            sum(purchase_plans.mapped("total_to_purchase_qty"))
            if purchase_plans
            else 0.0
        )
        total_to_move_qty = (
            sum(purchase_plans.mapped("total_to_move_qty")) if purchase_plans else 0.0
        )
        ...
        if selected_plan:
            plan_payload = {
                ...
                "warehouses": ", ".join(
                    selected_plan.warehouse_ids.mapped("display_name")
                ),
            }
        ...
        return {
            ...
            "kpis": {
                ...
                "workcenters_running": len(
                    [wc for wc in workcenter_data if wc["running_count"]]
                ),
            ...
            "alerts": {
                ...
                "manufacture_without_bom": (
                    len(
                        active_lines.filtered(
                            lambda l: l.action_manufacture and not l.bom_id
                        )
                    )
                ...
                "undefined_action": len(
                    active_lines.filtered(
                        lambda l: not (
                            l.action_manufacture or l.action_purchase or l.action_move
                        )
                    )
                ),
            },
    ...
    def _dashboard_permissions(self):
        user = self.env.user
        return {
            "manufacturing": user.has_group(
                "mrp_advanced_planner.group_planner_manufacturing_user"
            ),
            "purchase": user.has_group(
                "mrp_advanced_planner.group_planner_purchase_user"
            ),
        }
    ...
    def _check_dashboard_permission(self, plan_id, plan_type):
        if not self._dashboard_permissions().get(plan_type):
            raise AccessError(
                _("No tiene permisos para consultar esta sección del panel.")
            )
        if plan_id:
            plan = self.env["mrp.planning.plan"].browse(int(plan_id)).exists()
            if not plan or plan.plan_type != plan_type:
                raise AccessError(
                    _("La planificación seleccionada no corresponde a esta sección.")
                )
        return True
    ...
    def _serialize_mo(self, mo, state_labels):
        ...
        return {
            ...
            "remaining": round(max(qty - produced, 0), 2),
            "uom": mo.product_uom_id.name or "",
            "state": mo.state,
            "state_label": state_labels.get(mo.state, mo.state),
            "progress": progress,
            "warehouse": (
                mo.picking_type_id.warehouse_id.display_name
                if mo.picking_type_id.warehouse_id
                else ""
            ),
        }
                    ...
                    "orders": [
                        {
                            ...
                            "progress": round(wo.progress or 0.0, 1),
                            "plan": (
                                wo.production_id.advanced_plan_id.name
                                if wo.production_id.advanced_plan_id
                                else ""
                            ),
                        }
                        for wo in orders[:8]
    ...
    def action_open_transfers(self, plan_id=False):
        ...
        return {
            "type": "ir.actions.act_window",
            "name": _("Reabastecimientos del plan"),
    ...
    def action_open_workorders(self, workcenter_id=False, plan_id=False):
        self._check_dashboard_permission(plan_id, "manufacturing")
        domain = [
            ("production_id.advanced_plan_id", "=", int(plan_id) if plan_id else 0),
            ("state", "not in", ("done", "cancel")),
        ]
        if workcenter_id:
