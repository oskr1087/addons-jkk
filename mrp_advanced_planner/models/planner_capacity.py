...
class MrpAdvancedOperation(models.Model):
    _name = "mrp.advanced.operation"
    _description = "Finite Capacity Operation"
...
class MrpAdvancedRoll(models.Model):
    _name = "mrp.advanced.roll"
    _description = "Physical Roll Availability"
    name = fields.Char(required=True)
    ...
    def _compute_available(self):
        for rec in self:
            rec.available_length = max(rec.length - rec.reserved_length, 0)
