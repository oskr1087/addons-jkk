...
class PlanningOperation(models.Model):
    ...
    production_proposal_id = fields.Many2one(
        "mrp.planning.production.proposal", ondelete="cascade", index=True
    )
    ...
    state = fields.Selection(
        [("planned", "Planned"), ("conflict", "Conflict"), ("cancelled", "Cancelled")],
        default="planned",
        index=True,
    )
    load_hours = fields.Float(compute="_compute_load_hours", store=True)
    calendar_id = fields.Many2one(
        related="workcenter_id.resource_calendar_id", store=True
    )
...
class PlanningWorkcenterLoad(models.Model):
    ...
    state = fields.Selection(
        [("ok", "Available"), ("warning", "Warning"), ("overloaded", "Overloaded")],
    ...
    def _compute_utilization(self):
        for load in self:
            load.utilization = (
                load.load_hours / load.available_hours if load.available_hours else 0.0
            )
    ...
    def _compute_state(self):
        for load in self:
            load.state = (
                "overloaded"
                if load.utilization > 1.0
                else "warning" if load.utilization >= 0.85 else "ok"
            )
