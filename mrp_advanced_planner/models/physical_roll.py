...
class PhysicalRoll(models.Model):
    ...
    assigned_qty = fields.Float(default=0.0)
    available_qty = fields.Float(compute="_compute_available", store=True)
    assignment_ids = fields.One2many("mrp.physical.roll.assignment", "roll_id")
    cut_ids = fields.One2many("mrp.physical.roll.cut", "roll_id")
    assigned_qty_total = fields.Float(compute="_compute_assignment_totals", store=True)
    waste_qty_total = fields.Float(compute="_compute_assignment_totals", store=True)
    ...
    def _compute_available(self):
        for record in self:
            record.available_qty = (
                max(
                    record.physical_qty - record.reserved_qty - record.assigned_qty, 0.0
                )
                if record.state not in ("mounted", "consumed", "released")
                else 0.0
            )

    @api.depends(
        "assignment_ids.quantity",
        "assignment_ids.consumed_qty",
        "assignment_ids.waste_qty",
    )
    def _compute_assignment_totals(self):
        for record in self:
            active = record.assignment_ids.filtered(
                lambda item: item.state != "released"
            )
    ...
    def _sync_assignment_state(self):
        for record in self:
            if record.state in ("mounted", "released"):
                continue
            active = record.assignment_ids.filtered(
                lambda item: item.state in ("reserved", "assigned", "consumed")
            )
    ...
    def action_release(self):
        self.ensure_one()
        if self.assignment_ids.filtered(
            lambda item: item.state in ("assigned", "consumed")
        ):
    ...
    def action_reserve(self, quantity, plan=False, production=False, workcenter=False):
        ...
        assignment = self.env["mrp.physical.roll.assignment"].create(
            {
                "roll_id": self.id,
