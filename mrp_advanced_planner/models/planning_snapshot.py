...
class PlanningSnapshot(models.Model):
    ...
    state = fields.Selection([('created', 'Created'), ('restored', 'Restored')], default='created', required=True)
    ...
    def action_restore(self):
        for snapshot in self:
            for production in snapshot.production_ids.filtered(lambda record: record.state not in ('done', 'cancel')):
                production.action_cancel()
            for purchase in snapshot.purchase_ids.filtered(lambda record: record.state not in ('done', 'cancel')):
