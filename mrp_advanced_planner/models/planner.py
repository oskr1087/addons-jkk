...
class MrpAdvancedPlan(models.Model):
    ...
    warehouse_id = fields.Many2one('stock.warehouse', required=True, default=lambda self: self.env['stock.warehouse'].search([('company_id', '=', self.env.company.id)], limit=1))
    date_start = fields.Datetime(required=True, default=fields.Datetime.now)
    date_end = fields.Datetime(required=True)
    state = fields.Selection([('draft','Draft'),('queued','Queued'),('running','Running'),('calculated','Calculated'),('approved','Approved'),('applied','Applied'),('cancelled','Cancelled'),('failed','Failed')], default='draft', required=True)
    ...
    priority = fields.Selection([('sale','Sales priority'),('date','Due date'),('customer','Customer priority')], default='date')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('mrp.advanced.plan') or 'New'
...
class MrpAdvancedExecution(models.Model):
    ...
    state = fields.Selection([('queued','Queued'),('running','Running'),('completed','Completed'),('failed','Failed'),('cancelled','Cancelled')], default='queued')
    started_at = fields.Datetime(); finished_at = fields.Datetime(); message = fields.Text()
...
class MrpAdvancedConflict(models.Model):
    ...
    plan_id = fields.Many2one('mrp.advanced.plan', required=True, ondelete='cascade')
    severity = fields.Selection([('info','Info'),('warning','Warning'),('error','Error')], default='warning', required=True)
    conflict_type = fields.Selection([('material','Material'),('capacity','Capacity'),('calendar','Calendar'),('bom','BOM'),('route','Route'),('roll','Roll')], required=True)
    product_id = fields.Many2one('product.product'); workcenter_id = fields.Many2one('mrp.workcenter')
    message = fields.Text(required=True); resolved = fields.Boolean(default=False)
